#!/bin/bash
# Akij Infosec Team - Zabbix 7.4 for SUSE Linux Enterprise Server 15
RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; BLUE='\033[0;34m'; NC='\033[0m'
clear
echo -e "${BLUE}################################################################"
echo "#                  AKIJ INFOSEC TEAM                           #"
echo "#                ZABBIX 7.4 (SLES 15 SETUP)                    #"
echo "################################################################${NC}"
[ "$EUID" -ne 0 ] && echo -e "${RED}Run as root.${NC}" && exit 1
zypper --non-interactive update
zypper --non-interactive install wget curl openssl firewalld
rpm -Uvh https://repo.zabbix.com/zabbix/7.4/release/sles/15/x86_64/zabbix-release-latest-7.4.sles15.noarch.rpm
...
if ! command -v psql &> /dev/null; then zypper --non-interactive install postgresql15-server; systemctl enable --now postgresql; fi
...
echo "Zabbix Pass: ${DB_PASSWORD}" > /root/zabbix_sles_creds.txt
