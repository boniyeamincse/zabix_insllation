...
ufw allow ssh; ufw allow 80/tcp; ufw allow 443/tcp; ufw allow 10050/tcp; ufw allow 10051/tcp; echo "y" | ufw enable
systemctl restart zabbix-server zabbix-agent2 apache2
systemctl enable zabbix-server zabbix-agent2 apache2
IP_ADDR=$(hostname -I | awk '{print $1}')
echo -e "${GREEN}SUCCESS! UI: http://${IP_ADDR}/zabbix | DB Pass: ${DB_PASSWORD}${NC}"
echo "Zabbix Pass: ${DB_PASSWORD}" > /root/zabbix_debian_creds.txt
